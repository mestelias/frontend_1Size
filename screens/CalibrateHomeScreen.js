...
export default function CalibrateHomeScreen({ navigation }) {
  ...
  //Fonction qui permet de connaitre le type sélectionné dans le carrousel
  const getSelectedType = () => {
    const selectedItem = dataType[activeIndex];
    return selectedItem.text.toLowerCase();
  };
  ...
  const submitCalidrage = () => {
    ...
    fetch(`${url}/users/mensurations/${type}/${userToken}`)
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
        data.result && setModalVisible(true);

        data.error && navigation.navigate("Calibrate");
      });
  };
